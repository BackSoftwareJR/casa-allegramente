...
import { siteConfig, assets, navigation, secondaryNavigation, legalLinks, visitCta } from '@/data/content';
import LocationMap from '@/components/ui/location-map';

const resourceLinks = [
  { label: 'Servizi e retta', href: '/servizi' },
  { label: 'Blog', href: '/blog' },
  { label: 'Prenota una visita', href: '/contatti' },
];

/* Scotch-tape SVG — colore terracotta brand */
...
export default function Footer() {
  ...
  return (
    <footer role="contentinfo">
      ...
      <div className="border-t border-linen-200 bg-linen-50">
        <div className="container-site py-10 md:py-12">
          <div className="flex flex-col items-start gap-6 rounded-3xl border border-linen-200/80 bg-white p-8 shadow-warm-sm md:flex-row md:items-center md:justify-between md:p-10">
            <div>
              <p className="font-sans text-xs font-semibold uppercase tracking-[0.18em] text-primary">
                Gratuita · Senza impegno
              </p>
              <h2
                className="mt-2 font-display font-bold text-warm-brown"
            ...
            <div className="flex w-full shrink-0 flex-col gap-2 sm:w-auto sm:min-w-[280px]">
              <a
                ...
                className="group flex items-center gap-3 rounded-xl border border-linen-200 bg-linen-50/80 px-3.5 py-2.5 transition-all hover:border-primary/30 hover:bg-white hover:shadow-warm-sm"
              >
                <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-primary/10 text-primary group-hover:bg-primary group-hover:text-white">
                  <Phone size={15} strokeWidth={2.25} />
                </span>
                <span className="min-w-0 flex-1 text-left">
                  <span className="block font-sans text-sm font-semibold text-warm-brown">{contact.phoneCtaLabel}</span>
                  <span className="block truncate font-sans text-xs text-ink-muted">{contact.phone}</span>
                </span>
                <ArrowRight size={14} className="shrink-0 text-ink-muted/40 group-hover:translate-x-0.5 group-hover:text-primary" />
              </a>
              <a
                href={`https://wa.me/${contact.whatsapp}?text=Buongiorno%2C%20vorrei%20prenotare%20una%20visita`}
                target="_blank"
                rel="noopener noreferrer"
                className="group flex items-center gap-3 rounded-xl border border-linen-200 bg-linen-50/80 px-3.5 py-2.5 transition-all hover:border-[#25D366]/35 hover:bg-white hover:shadow-warm-sm"
              >
                <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-[#25D366]/10 text-[#25D366] group-hover:bg-[#25D366] group-hover:text-white">
                  <MessageCircle size={15} strokeWidth={2.25} />
                </span>
                <span className="min-w-0 flex-1 text-left">
                  <span className="block font-sans text-sm font-semibold text-warm-brown">WhatsApp</span>
                  <span className="block font-sans text-xs text-ink-muted">Scrivici quando vuoi</span>
                </span>
                <ArrowRight size={14} className="shrink-0 text-ink-muted/40 group-hover:translate-x-0.5 group-hover:text-[#25D366]" />
              </a>
            </div>
          </div>
        </div>
      ...
      {/* ── Main footer — sfondo bianco panna ─────────────────────────── */}
      <div className="bg-linen-100">
        <div className="container-site py-10 md:py-14">

          {/* Card con nastro adesivo */}
          <div className="relative rounded-3xl border border-linen-200/80 bg-white px-6 py-10 shadow-warm-md md:px-10 md:py-12">

            {/* Nastro angolo superiore sinistro */}
            <div className="absolute -top-5 -left-4 z-10 scale-75 origin-top-left">
              <Tape />
            </div>
            {/* Nastro angolo superiore destro */}
            <div className="absolute -top-5 -right-4 z-10 scale-75 origin-top-right rotate-90">
              <Tape />
            </div>
            ...
            <div className="grid gap-10 md:grid-cols-12">
              ...
              <div className="md:col-span-12 lg:col-span-4">
                <Link href="/" className="group inline-flex items-center gap-4">
                  <Image
                    ...
                    width={140}
                    height={140}
                    className="h-28 w-auto transition-opacity group-hover:opacity-90 md:h-32"
                  />
                  <div>
                    <span
                      className="block font-display font-extrabold leading-tight text-warm-brown"
                      style={{ fontSize: 'clamp(1.4rem, 2.5vw, 1.9rem)' }}
                    >
                      Casa<br />
                      <span className="text-primary">Allegramente</span>
                    </span>
                  </div>
                </Link>
                ...
                <blockquote className="mt-5">
                  <p
                    className="font-sans italic leading-snug text-primary/80"
                ...
                <ul className="mt-6 space-y-2.5">
                  {[
                    { icon: Phone, label: contact.phone, note: contact.phoneContactNote, href: `tel:${contact.phoneRaw}` },
                    { icon: MessageCircle, label: contact.whatsappDisplay, href: `https://wa.me/${contact.whatsapp}` },
                    { icon: Mail, label: contact.email, href: `mailto:${contact.email}` },
                  ].map(({ icon: Icon, label, note, href }) => (
                    <li key={href}>
                      <a
                        href={href}
                        target={href.startsWith('http') ? '_blank' : undefined}
                        rel={href.startsWith('http') ? 'noopener noreferrer' : undefined}
                        className="group inline-flex items-start gap-2.5 font-sans text-sm text-ink-muted transition-colors hover:text-primary"
                      >
                        <Icon size={13} className="mt-0.5 text-primary/50 transition-colors group-hover:text-primary" />
                        <span>
                          {label}
                          {note && <span className="mt-0.5 block text-xs text-ink-muted/80">{note}</span>}
                        </span>
                      </a>
                    </li>
                  ))}
              ...
              <div className="md:col-span-4 lg:col-span-2">
                <h3 className="font-sans text-[10px] font-semibold uppercase tracking-[0.2em] text-primary/70">
                  Navigazione
                </h3>
                <ul className="mt-5 space-y-2">
                  {navigation.map((item) => (
                    <li key={item.href}>
                      <Link
                        href={item.href}
                        className="group inline-flex items-center gap-2 font-sans text-sm text-ink-muted transition-colors hover:text-warm-brown"
                      >
                ...
                <h3 className="mt-8 font-sans text-[10px] font-semibold uppercase tracking-[0.2em] text-primary/70">
                  Link utili
                </h3>
                <ul className="mt-5 space-y-2">
              ...
              <div className="md:col-span-12 lg:col-span-2">
                ...
                <div className="mt-5 space-y-3">
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="group flex items-start gap-2.5 text-sm text-ink-muted transition-colors hover:text-warm-brown"
                  >
                    <MapPin size={14} className="mt-0.5 shrink-0 text-primary/50 group-hover:text-primary" />
                    <span className="font-sans leading-snug">
                      {contact.address.street}<br />
                      {contact.address.cap} {contact.address.city} ({contact.address.province})
                    </span>
                  </a>
                  <div className="flex items-start gap-2.5 font-sans text-sm text-ink-muted">
                    <Clock size={13} className="mt-0.5 shrink-0 text-primary/50" />
                    <span className="leading-snug">{contact.hours}</span>
                  </div>
                </div>
                <div className="mt-5">
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    aria-label={`Apri mappa ${siteConfig.name} su Google Maps`}
          ...
          {/* ── Bottom bar ──────────────────────────────────────────── */}
          <div className="mt-8 flex flex-col items-center gap-4 border-t border-linen-200/80 pt-8">
            <div className="text-center">
              <p className="font-sans text-xs text-ink-muted">
                © {currentYear} {siteConfig.name} — gestito da {legal.companyNameFull}
              </p>
              <p className="mt-1 font-sans text-xs text-ink-muted/80">
                P.IVA {siteConfig.piva} · PEC{' '}
                <a href={`mailto:${legal.pec}`} className="text-primary/80 hover:text-primary hover:underline">
                  {legal.pec}
                </a>
                · Legale rappresentante: {legal.legalRepresentative}
              </p>
            </div>
            <div className="flex flex-wrap justify-center gap-x-5 gap-y-2">
              {legalLinks.map((l) => (
                <Link
                  key={l.href}
                  href={l.href}
                  className="font-sans text-xs text-ink-muted transition-colors hover:text-warm-brown"
                >
                  {l.label}
                </Link>
              ))}
              <span className="font-sans text-xs text-ink-muted/70">
                Realizzato da{' '}
                <a
